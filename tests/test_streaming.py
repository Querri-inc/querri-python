...
class TestChatStream:
    ...
    def _make_response(
        self, lines: list[str], headers: dict[str, str] | None = None
    ) -> httpx.Response:
    ...
    def test_newline_unescaping(self):
        """Verify that escaped \\n sequences in SSE text
        chunks are unescaped to real newlines."""
        response = self._make_response(
            [
                '0:"line1\\nline2"',
                "d:done",
    ...
    def test_done_signal_stops_iteration(self):
        """Verify that a 'd:' done signal stops
        iteration, ignoring subsequent chunks."""
        response = self._make_response(
            [
                '0:"a"',
                "d:done",
    ...
    def test_response_closed_after_iteration(self):
        """Verify that the underlying HTTP response is
        closed when iteration completes."""
        response = self._make_response(["d:done"])
        stream = ChatStream(response)
        list(stream)
    ...
    def test_unquoted_text_chunk(self):
        """Verify that text chunks without surrounding
        quotes are passed through as-is."""
        response = self._make_response(
            [
